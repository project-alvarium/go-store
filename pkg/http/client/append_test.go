...
// TestInstance_Append tests Append client method.
func TestInstance_Append(t *testing.T) {
	...
	cases := []testCase{
		{
			...
			test: func(t *testing.T) {
				...
				result := sut.Append(url.New(test.FactoryRandomString()), nil)

				assert.Equal(t, appendRequestorFailure, result)
			},
		},
		{
			...
			test: func(t *testing.T) {
				...
				assert.Equal(t, appendUnmarshalFailure, result)
			},
		},
		{
			name: "success",
			test: func(t *testing.T) {
				...
				result := sut.Append(idContract, m)

				assert.Equal(t, append.Method, requestor.RequestMethod)
				assert.Equal(t, append.EscapedRoute(idContract), requestor.RequestURL)
				assert.Equal(t, testInternal.Marshal(t, m), requestor.RequestBody)
				assert.Equal(t, appendSuccess, result)
			},
		},
	}
